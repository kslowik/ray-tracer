...
fn test_to_json() {
    let config = Config {
        ...
        objects: vec![Sphere::new(
            Point3D::new(0.0, 0.0, -1.0),
            0.5,
            Material::Lambertian(Lambertian::new(Srgb::new(0.8f32, 0.3f32, 0.3f32))),
        )],
    };
...
fn _make_cover_world() -> Vec<Sphere> {
    ...
    world.push(Sphere::new(
        ...
        1.0,
        Material::Lambertian(Lambertian::new(Srgb::new(0.4f32, 0.2f32, 0.1f32))),
    ));
    world.push(Sphere::new(
        Point3D::new(4.0, 1.0, 0.0),
        1.0,
        Material::Metal(Metal::new(Srgb::new(0.7f32, 0.6f32, 0.5f32), 0.0)),
    ));
    world
}
...
fn test_cover_scene_to_json() {
    ...
    fs::write("/tmp/impressive_scene.json", serialized).unwrap();
}
